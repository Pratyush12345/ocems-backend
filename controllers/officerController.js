...
module.exports.updateOfficer = (req,res) => {
    const adminuid = req.userData.uid
    const officeruid = req.params.officeruid
    ...
    .then(user => {
        if(user.exists && user.get('roleName') === "Admin"){
            ...
            .then(async plant => {
                ...
                await firestore.collection('users').doc(officeruid).update(updatedData)
...
module.exports.deleteOfficer = (req,res) => {
    const adminuid = req.userData.uid
    const officeruid = req.params.officeruid
    ...
    .then(user => {
        if(user.exists && user.get('roleName') === "Admin"){
            ...
            .then(async plant => {
                ...
                await firebase.auth().deleteUser(officeruid)
                
                await firestore.collection('users').doc(officeruid).delete()
